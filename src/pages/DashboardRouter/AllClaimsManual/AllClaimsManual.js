...
const AllClaimsManual = () => {
  ...
  const columns = [
    ...
    {
      title: "قيمة التحمل",
      dataIndex: "enduranceRatio",
    },
    {
      title: "أتعاب الشركة",
      dataIndex: "companyFees",
    },
    {
      title: "المبلغ الصافي",
      dataIndex: "nonAdd",
  ...
  const handleEdit = () => {
    form
      .validateFields()
      .then((values) => {
        const surgicalId = data11.find((item) => item.name === values.nameS)?.id;
        const hospitalId = data12.find((item) => item.name === values.nameH)?.id;
        const personId = data13.find((item) => item.ensuranceNumber === values.ensurNum)?.id;
        ...
        const updatedValues = {
          ...
          send: true,
          loginDate: values.loginDate ? values.loginDate.format('YYYY-MM-DD') : null,
          exitDate: values.exitDate ? values.exitDate.format('YYYY-MM-DD') : null,
          personId: personId,
        };

        axios
          .put(`${config.baseUrl1}/${config.recovereds}/${editingUnit.id}`, updatedValues)
          .then(() => {
            notification.success({
            ...
            setData((prevData) =>
              prevData.map((item) =>
                item.id === editingUnit.id
                  ? { ...item, ...updatedValues, nameS: values.nameS, nameH: values.nameH }
                  : item
              )
            );
            setEditingUnit(null);
    ...
    <BackButton/>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleEdit}>
          ...
          <Form.Item name="ensurNum" label="الرقم التأميني"
   rules={[
    { required: false },
    { pattern: /^\d+$/, message: "يرجى إدخال أرقام فقط" },
          ...
          <Form.Item name="totalPrice" label="قيمة المطالبة"
           rules={[
            { required: false },
            { pattern: /^\d+$/, message: "يرجى إدخال أرقام فقط" },
          ]}>
          ...
          <Form.Item name="approvedPrice" label="الموافق عليه"
           rules={[
            { required: false },
            { pattern: /^\d+$/, message: "يرجى إدخال أرقام فقط" },
          ]}>
          ...
          <Form.Item name="enduranceRatio" label="قيمة التحمل" 
           rules={[
            { required: false },
            { pattern: /^\d+$/, message: "يرجى إدخال أرقام فقط" },
          ]}
          ...
          <Form.Item name="companyFees" label="أتعاب الشركة" 
           rules={[
            { required: false },
            { pattern: /^\d+$/, message: "يرجى إدخال أرقام فقط" },
          ]}>
          ...
          <Form.Item name="nonAdd" label="المبلغ الصافي"
           rules={[
            { required: false },
            { pattern: /^\d+$/, message: "يرجى إدخال أرقام فقط" },
          ]}
          ...
          <Form.Item name="nonAddForPerson" label="الصافي للمؤمن" 
           rules={[
            { required: false },
            { pattern: /^\d+$/, message: "يرجى إدخال أرقام فقط" },
          ]}>
...
export default AllClaimsManual;
