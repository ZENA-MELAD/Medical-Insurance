...
const AllRenewal = ({ personId ,name,last}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
     ...
     `     ${config.baseUrl1}/${config.GetBenifites}/${personId}`
        );
  ...
  const columns = [
    {
      title: `جدول التسجيل/عدم التسجيل/الاستفادة/عدم الاستفادة لعقد نقابة المهندسين حمص للمهندس "${name} ${last}"` ,
      children: [
        {
          title: "العام",
          dataIndex: "year",
